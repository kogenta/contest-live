...
function loadContestantsForJudge() {
    ...
    contestantList.innerHTML = '<div class="loading">Loading contestants...</div>';
    ...
    database.ref('contestants').on('value', (snapshot) => {
        ...
        // Sort by number
        const sortedContestants = Object.entries(contestants).sort((a, b) => a[1].number - b[1].number);
...
function createContestantCard(contestantId, contestant) {
    ...
    database.ref(`scores/${contestantId}/${currentJudgeId}`).once('value', (snapshot) => {
        if (snapshot.exists()) {
    ...
    CRITERIA.forEach(criteria => {
        ...
        setTimeout(() => {
            const rangeInput = document.getElementById(`range-${contestantId}-${criteria.id}`);
            rangeInput.addEventListener('input', function() {
                const value = parseInt(this.value);
                document.getElementById(`score-${contestantId}-${criteria.id}`).textContent = `${value}/${criteria.max}`;
    ...
    totalDiv.textContent = `Total: ${totalScore}/${CRITERIA.reduce((sum, c) => sum + c.max, 0)}`;
            ...
            CRITERIA.forEach(criteria => {
                if (existingScores[criteria.id] !== undefined) {
                    ...
                    if (rangeInput) {
                        ...
                        document.getElementById(`score-${contestantId}-${criteria.id}`).textContent = 
                            `${existingScores[criteria.id]}/${criteria.max}`;
                    }
                }
            });
            updateTotalScore(contestantId);
...
function submitScore(contestantId) {
    ...
    // Save to Firebase
    database.ref(`scores/${contestantId}/${currentJudgeId}`).set(scores)
        .then(() => {
...
function deleteContestant(contestantId) {
    if (!confirm('Are you sure you want to delete this contestant? This will also delete all scores.')) {
        return;
    }
    ...
    database.ref('contestants').once('value', (contestantsSnapshot) => {
        ...
        database.ref('scores').once('value', (scoresSnapshot) => {
            ...
            Object.entries(contestants).forEach(([contestantId, contestant]) => {
                ...
                const avgScore = judgeCount > 0 ? (totalScore / judgeCount).toFixed(2) : 0;
            ...
            rankings.forEach((contestant, index) => {
                ...
                rankItem.innerHTML = `
                    ${rankDisplay}
                    <div class="rank-info">
                        <div class="rank-name">#${contestant.number} - ${contestant.name}</div>
